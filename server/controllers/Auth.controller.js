...
class Auth {
  async registration(req, res) {
    try {
      ...
      return res.status(200).json({ ok: true, message: "Пользователь зарегистрирован", type: "success", });
    } catch (err) {
      console.log(err);
  ...
  async login(req, res) {
    try {
      ...
      if (!isTruePassword) {
        return res.status(400).json({ ok: false, message: "Неверный пароль", type: "error", });
      }

      const token = jwt.sign(user.dataValues, process.env.SECRET_KEY, { expiresIn: Math.floor(Date.now() / 1000) + (60 * 60), });

      return res.status(200).json({ ok: true, message: "Пользователь вошел в систему", token: `Bearer ${token}`, type: "success", });
    } catch (err) {
      console.log(err);
